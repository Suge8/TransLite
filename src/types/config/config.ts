...
import { langCodeISO6393Schema, langLevel } from "@/utils/languages/definitions"
...
const siteControlSchema = z.strictObject({
...
export const configSchema = z.strictObject({
  ...
  if (data.languageDetection.mode === "llm") {
    const providerId = data.languageDetection.providerId
    if (!providerId) {
      ctx.addIssue({
        code: "custom",
    ...
    if (!provider) {
      ctx.addIssue({
        code: "custom",
        message: `Language detection provider "${providerId}" not found in providersConfig.`,
        path: ["languageDetection", "providerId"],
      })
      return
    }

    if (!isLLMProvider(provider.provider)) {
      ctx.addIssue({
        code: "custom",
        message: `Language detection provider "${providerId}" is not an LLM provider.`,
        path: ["languageDetection", "providerId"],
      })
    }
    if (!provider.enabled) {
      ctx.addIssue({
        code: "custom",
        message: `Language detection provider "${providerId}" must be enabled.`,
        path: ["languageDetection", "providerId"],
      })
    }
  }
