...
| Real-browser verification workflow, debugging heuristics, translite capture recipe | [references/workflow.md](references/workflow.md) |
...
For translite page translation, a reliable automation path was:
...
6. Wait for `.tl-spinner` nodes and record live DOM evidence.
...
Good loading-time evidence for translite:
...
const loadingEvidence = await page.evaluate(() => ({
  spinnerCount: document.querySelectorAll('.tl-spinner').length,
  sampleSpinnerStyles: Array.from(document.querySelectorAll('.tl-spinner'))
    .slice(0, 3)
    .map(node => node.getAttribute('style')),
}));
```

Good completion-time evidence for translite:

```js
await page.waitForFunction(
  () => {
    const wrappers = Array.from(document.querySelectorAll('.tl-translated-content-wrapper'));
...
- See `references/workflow.md` for the real-browser checklist, debugging heuristics, and translite capture recipe.
